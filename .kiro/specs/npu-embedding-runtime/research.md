...
- **Selected Approach**: Option 2. Backends return token embeddings **alone**, and a single shared post-processing stage combines them with the attention mask **the caller already holds** to produce the final vector. *(Corrected 2026-09-06, task 5.2. This originally read "token embeddings plus the attention mask", which design.md's TransformerBackend correction superseded: the mask is the caller's own argument, so returning a copy would create a second mask that could disagree with the first after a padding change — precisely where masked mean pooling breaks silently — and the isolated adapter would have had to serialize it back across the socket for nothing.)*
