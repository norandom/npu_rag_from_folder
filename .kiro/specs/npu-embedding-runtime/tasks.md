...
- [x] 5.5 Replace the third candidate model and generalise pooling
...
- [x] 6.1 (P) Build the committed benchmark fixture
  ...
  - **Excerpt cap, decided 2026-09-07.** The archive is 1205 markdown files from three Substack publications, and this repository pushes to public GitHub, so the fixture carries **short excerpts only** - roughly 500 characters per chunk, and only a few chunks drawn from any one source article, so that no article is substantially reproduced. Record each chunk's source publication and article so the composition requirement 6.5 asks for is real.
  - **This costs the benchmark nothing measurable, and the reason is worth keeping.** The NPU graph is compiled at a static `(batch_size, 512)`, so every input is padded to the compiled length and consumes identical compute regardless of its actual text length. Throughput, latency, energy and peak memory are therefore unaffected by excerpt length - only retrieval quality (6.6) sees the text at all, and a ~500-character excerpt is a substantive paragraph that embeds meaningfully. Do **not** "improve" the fixture later by lengthening chunks on the assumption that longer inputs are more realistic; they are not more expensive, and they are more of someone else's writing.
...
- [x] 6.2 (P) Implement energy sampling with an explicit unavailable case
...
- [x] 6.3 Drive the model and provider matrix and capture per-run metrics
  ...
  - **Scope split from 7.2, decided 2026-09-07.** 6.3 builds and proves the harness; **7.2 pays for the full matrix run.** Nothing is persistently prepared today - 5.5's compile went to a scratchpad - so a full pass means cold-compiling all three models, and the one measured data point is 2953 s (~49 min) for the 573 MB candidate. EmbeddingGemma is 1.22 GB and has never been compiled. That is a 2.5-5+ hour job and it belongs in one deliberate window, not in the middle of the task queue. **6.3 must therefore not trigger a cold compile of a model that is not already prepared.** Prove the harness against the CPU provider and against warm or injected NPU artifacts, and make the matrix driver take its cell list as data so 7.2 can hand it the full set unchanged.
  - **Do not run compiles concurrently with anything else.** Task 6.2 measured that under machine saturation an `xrt-smi` poll goes from ~0.16 s to ~1.0 s and whole telemetry bursts come back empty (6 of 8). A compile running beside a measured cell would corrupt exactly the energy figure this task exists to capture.
...
- [x] 6.7 Render the benchmark document - **DESCOPED 2026-09-07**
  ...
  - **Render `LATENCY_METHODOLOGY`, `MEMORY_METHODOLOGY` and `EnergyMeasurement.methodology` verbatim** (added 2026-09-07). Each states a confound the figure beside it cannot express: latency is ~78% ORT session construction rather than inference, peak memory is process-wide and never baseline-subtracted, and energy understates when a poll is missed. Requirement 6.8 forbids substituting an estimate, so stating the confound is the honest alternative - and an unrendered caveat is worse than an estimate, because the number then reads as clean.
  - **Close review's H6/H7 here** (routed from 6.3, 2026-09-07): the two latency shares are each bound to their own term but their *relation* is asserted nowhere, so a transposed pair claiming inference dominates passes, as does deleting "That per-call construction dominates the figure". Parse both percentages and assert construction exceeds inference, number-tolerant so 7.2's re-measure passes. If a future NPU measurement ever showed construction *not* dominating, firing would be correct - the whole caveat would need rewriting.
...
- [x] 8.4 (P) Harden the remaining prose assertions across `bench/` - **DESCOPED 2026-09-07**
  ...
  - Observable: for each assertion changed, a planted mutant that degrades the string while satisfying the old form is killed by the new one, and a mutant representing a legitimate future edit (a re-measured figure) still passes
  - _Requirements: 6.2, 6.8_
...
- 5.1: consequence for every future module - adding a file under `src/npu_rag/embedding/` now requires a deliberate `LAYER_ORDER` placement or the suite fails. That is the intent; do not "fix" it by deleting the check. *(**Overstated; corrected by 6.1, 2026-09-07.** True only for TOP-LEVEL modules and NEW subpackages. The guard computes a module's rank as `relative.parts[0] if len(relative.parts) > 1 else relative.stem`, so a file added inside an existing subpackage - `environment/`, `models/`, `providers/`, `bench/`, i.e. most of the package - silently inherits that subpackage's rank and forces no decision. Confirmed empirically by 6.1's reviewer: a decoy at `bench/decoy.py` left the suite green, while a decoy at the top level failed the guard as advertised. Directory granularity actually matches design.md's stated dependency direction, which names subpackages rather than files, so the honest fix is probably to narrow this note rather than to tighten the guard - but **do not rely on the guard to catch a bad import inside a subpackage**, because it will not.)*
...
- 5.5: **BINDING ON 6.3 AND 6.4 - COMPILE COST.** Measured for `gte-modernbert-base`: acquisition 221.6 s (2.53 GB snapshot for a 0.6 GB model - the repo ships eight prebuilt ONNX variants this project never uses, since it exports its own graph; an `allow_patterns` restriction would cut ~1.9 GB), export 25.0 s, **NPU compile 2953.5 s cold / 0.59 s warm**. Requirement 4.1's "around half the compile time of anything else in the set" was an unvalidated guess and is corrected in place. **A cold NPU cell for this model costs ~49 minutes**, so 6.3 must reuse warm artifacts across cells and 6.4 must reuse them across repetitions, or the matrix becomes a multi-hour job per pass. Warm reuse is proven (`reused=True`).
- 5.5: the model exports to a **fully static** graph with no `attn_implementation` override - `input_ids [1,512]`, `attention_mask [1,512]`, `token_embeddings [1,512,768]`, 1167 nodes, 572.7 MB single protobuf - and the compiler put **1149 of 1152 operators (99.740%) in one subgraph**, published node mix `{EPContext:1, Cast:1, Gather:1, GatherND:1}`. ModernBERT's alternating local/global attention was the risk going in; it did not materialise. Requirement 4.1's open question about external-data ONNX models remains **untested** - at 573 MB this candidate never approached the single-protobuf limit, so `external_data=False` was not a constraint here.
- 5.5: **`ModelProfile.pooling` HAD ZERO PRODUCTION CONSUMERS BEFORE THIS TASK.** `finalize` hardcoded masked mean pooling, so widening the `Literal` alone would have left a CLS profile silently mean-pooled - right shape, right norm, wrong meaning. The dispatch is now real: `postprocess.pool(tokens, mask, *, rule)` branches and otherwise raises `ExecutionError` naming the rule and the known set; `finalize` takes `pooling` keyword-only **with no default**, so no call site can fall back by omission; `cls_pool` takes token embeddings **only** and has no `attention_mask` parameter at all, which is what structurally prevents the mask-blind hazard returning. Generalises: **a declared field that nothing reads is not a contract, it is a comment.** Grep for other such fields before trusting one.
- 5.5: an unimplemented pooling rule fails at EMBED time, not at profile construction - `profiles.py` imports only `types` and validating against `POOLING_RULES` would couple it to `postprocess`. Upheld on review as correct for the task (it fails loudly, which is what was required), but it now costs a ~49-minute compile before the error surfaces. **If 6.3 adds a second `EmbeddingService` construction site, put the cheap gate there**, beside the Dense-stage refusal Note 5.4 added.
- 5.5: this is the first **symmetric** profile - document and query templates are both identity - so Note 5.1's ban on asserting that the two renderings differ is now load-bearing rather than theoretical. `test_tokenize_live.py` had exactly that assertion and it was falsified; it now asserts they are equal precisely when the templates are, with the spanning control (`test_the_lineup_spans_both_a_symmetric_and_an_asymmetric_convention`) placed in a NON-live test so CI, which has no NPU, still runs it. Also note `tests/embedding/test_tokenize.py`'s `plain` fixture is now fully symmetric: all ~35 current uses are document-side and unaffected, but reaching for it to prove *query*-side template application would be vacuous.
- 6.1: **THE EXCERPT CAP IS A PUBLISHING DECISION, ENFORCED IN THE LOADER ON EVERY READ.** The archive is third-party Substack content and this repository is public, so the project owner capped the fixture at ~500 characters per chunk and 2 chunks per article (decided 2026-09-07). `bench/corpus.py` re-checks both on every load, not just in the generator, so a regenerated or hand-edited fixture cannot quietly exceed them; the generator's constants are pinned to the loader's by a test, because the package may not import `tools/` and the pair would otherwise drift exactly as Note 1.5 records. Shipped: 200 chunks / 120 articles / 68,772 characters. **Do not raise the cap or lengthen chunks "for realism"** - the graph is compiled at a static `(batch, 512)` so every input costs identical compute whatever its length; longer excerpts buy no measurement fidelity and are simply more of someone else's writing.
- 6.1: **BINDING ON 6.6 - the query set defeats a lexical baseline, and that is the whole point.** 6.6's Observable is that a model with its Dense stage omitted "scores visibly worse"; if the queries could be answered by keyword overlap, 6.6 would prove nothing about embeddings. Measured: an IDF-weighted bag-of-words ranking scores mean recall@5 = **0.083** against a 0.35 ceiling, 12 of 17 queries at 0.00, with a control proving the same scorer reaches 1.000 on queries lifted from the chunks. The opposite failure is closed too - `test_the_relevance_labels_are_not_arbitrary` requires co-labelled chunks to be >= 2x as similar as random pairs (measured 3.6x). **If 6.6 finds a correct model scoring near zero at k=5, fix it on 6.6's side** - report nDCG or MRR, or widen k - never by lexically loosening the fixture.
- 6.1: **BINDING ON 6.3 - pass the title.** `CorpusChunk.article_title` exists because requirement 3.4's document template has a title slot and `DocumentText` carries one. Construct inputs as `DocumentText(content=chunk.text, title=chunk.article_title)`, or every fixture chunk embeds with the literal sentinel `none` in its title slot. Reviewed against Note 5.5's "a declared field that nothing reads is a comment": this one is validated on every load and read by two tests, so it is provenance with a named destination rather than an unread knob - but 6.3 is the destination, so wire it.
- 6.1: the fixture is **stratified with a disclosed floor**, not proportional. Archive: businessengineer.ai 849 (70.5%), nurijanian 187 (15.5%), quantbeckman.com 169 (14.0%). Sample articles: 71 / 24 / 25. `MINIMUM_ARTICLES_PER_PUBLICATION = 24` deliberately lifts the two smaller publications so they can support relevance judgements. Recorded in `composition.json`'s `sampling` field, so the deviation is a stated choice. 6.7 may want the archive's own distribution alongside the sample's so a reader can see the floor's effect.
- 6.1: a local absolute path leaked into the generator's usage docstring and was caught by the review's content scan. **The same scan found a pre-existing leak of the verifying machine's account name in `tools/provision_npu.py`'s `DEFAULT_NUGET_NATIVE_DIR`**, committed since task 1.2 and already pushed. Both are now written relative (`<path>` and `Path.home()`), which resolves identically here. **Any tooling default that names a real path is a candidate leak in a public repository** - prefer `Path.home()` or a required argument.
- 6.2: **FOLDING `N/A` TO 0.0 W PRODUCES AN IDENTICAL JOULE FIGURE.** `0.0 W x 0.5 s = 0 J`, so no energy assertion can ever catch the fold - only the accounting can (`missed_samples`, `reported_samples`, `measured_seconds`). That is precisely why Note 1.4 demanded a missed-sample count, and why `test_an_absent_reading_is_dropped_from_the_integral_never_folded_to_zero` asserts the honest and folded traces **side by side**: same joules, different counts. Any future change to the integral must keep that pairing.
- 6.2: design.md's `PowerSampler` sketch is superseded and design.md is corrected in place. `PowerSource` is a Protocol with `read_power() -> PowerReading` carrying three states, satisfied **structurally by the existing `XrtSmiWrapper` with no adapter**; `PowerSampler` is the concurrent collector - a single-use context manager owning its thread and joining on exit, which is what makes "never inside the measured call" structural rather than conventional. `supported()` is deliberately not reimplemented: `CapabilityReport.power_reporting_supported` already answers it and cannot be re-derived from samples, since a PHX part and this Strix part emit the same `N/A` token. Integration is left-endpoint rectangles over **observed** windows, never the nominal interval.
- 6.2: **BINDING ON 6.3.** Pass `power_reporting_supported` from the `CapabilityReport`, never hardcoded `True`, or a PHX machine gets the wrong omission text. Construct **one `PowerSampler` per measured cell** - it raises on reuse, like `RunTracker` (Note 2.3). Time the workload separately and state that the energy denominator is the sampler's slightly larger bracketing window; the sampler brackets the call by well under a millisecond, so it marginally overstates rather than understates.
- 6.2: **BINDING ON 6.7.** The integral **understates when polls are missed**, deliberately - rescaling to cover the gap would be the estimated value requirement 6.8 forbids. That only stays honest if the report renders `EnergyMeasurement.methodology`; otherwise the understatement is a silent caveat, which is worse than an estimate. Also note `PowerIntegral.unsupported_samples` is **not** carried onto `EnergyMeasurement`, so a mixed run publishes `samples_reported + samples_missed < samples_taken` with the shortfall visible but unattributed.
- 6.2: **A LIVE ASSERTION THAT THE HARDWARE COOPERATES IS NOT A TEST OF YOUR CODE.** This task shipped, was approved, and still had a flaky live test; the fresh-evidence completion gate caught it, not the review. `assert integral.reported_samples >= 1` failed as `assert 0 >= 1` because **every poll in the burst came back `N/A`** - the production code was right and declined to invent a figure, and the test rejected it. Measured under saturation: **6 of 8 bursts are entirely `N/A`**, so the assertion would have failed roughly three runs in four on this machine. The fix asserts the code property under **both** outcomes rather than requiring one: reported arm keeps the value assertions, absent arm demands `value is None` with the all-polls-empty reason specifically (not the unsupported text), `samples_missed == len(samples)` and `measured_seconds == 0.0`. That is strictly stronger - it gave the all-`N/A` path live coverage nothing had.
- 6.2: **CONTENTION CHANGES THE PHYSICS YOU ARE MEASURING.** One `xrt-smi` poll costs ~0.16 s at idle and **0.90-1.02 s under 24-way saturation** (n=12) on this 24-logical-processor machine. Note 1.4's "2 of 39 `N/A`" was measured at idle; under load whole bursts vanish. **Any live assertion calibrated on idle behaviour is calibrated on the wrong machine** - which matters directly for 6.3 and 7.2, since a benchmark measures a busy machine by definition. The second flake was fixed by making "the sampler polled repeatedly" a property of the sampler - the measured call waits for the poll count, with a 20 s ceiling that fails loudly rather than hanging - instead of raising a constant, which is just another bet on machine speed. **Do not "optimise" that ceiling down.**
- 6.2: two vacuity findings from review, both the standing lesson in new clothes. The fixture's `SERIES_INTERVAL` was `0.5`, numerically equal to `DEFAULT_SAMPLING_INTERVAL_SECONDS`, so both assertions that the *recorded* interval comes from the trace passed with the value hardcoded to the module default - the mutant survived all 52 unit tests and died only to a live test CI always skips. Now `0.4`, guarded by an explicit `SERIES_INTERVAL != DEFAULT_SAMPLING_INTERVAL_SECONDS` control. Separately, every unit sampler test used a 1 ms interval, so a sampler that ignored its wait and **polled in a tight loop** passed all 52 - the exact measurement perturbation this task exists to prevent.
- 6.3: **THE PUBLISHED SINGLE-INPUT LATENCY IS ~78% ORT SESSION CONSTRUCTION, NOT INFERENCE.** Requirement 2.7 binds a backend per operation and the factories open a fresh session on every call, so each probe pays one warm manifest read plus one session construction. Measured on the CPU control model: median 0.2426 s = 0.1889 s construction (77.8%) against 0.0299 s inference (12.3%), a 6.3x ratio. **The measurement is faithful and must not change** - design.md requires the production path with no privileged access, and a real caller does pay this. What was missing was the statement, so `LATENCY_METHODOLOGY` now carries it. **On the NPU the share will be larger and scales with artifact size rather than compute**, which matters because requirement 7.3 asks 6.7 to pick a default from these very figures: a confound that scales with file size can invert the ranking. The same construction sits once inside `workload_seconds`, so throughput understates steady-state rate, most at small sample sizes.
- 6.3: **BINDING ON 6.4 AND 7.2 - run 6.4 FIRST.** The harness aborts on a failing cell; design.md's Batch Contract says the matrix continues, and per-cell failure tolerance is 6.4's Observable. Until it lands, **one mis-bound NPU cell destroys a 2.5-5 hour run after paying its ~49-minute compile.** A cell can mis-bind for ordinary reasons - an unsatisfied condition, a weak partition share - and `CellMetrics` correctly refuses a record whose served provider is not the one the cell named.
- 6.3: what 7.2's first real NPU cell still risks, none of it covered today. The **Dense path has never met a real NPU artifact** - only the gated, never-compiled EmbeddingGemma declares `has_dense_stage`, and `load_dense_layers` raises *after* the compile is paid. **Per-probe EP-context re-open is untested at scale**: 10 latency probes per cell each construct a fresh Vitis AI session over a 573 MB-1.22 GB snapshot, which is where the 77.8% grows and where it may interact with concurrent `xrt-smi` polling. **Peak memory for an NPU cell includes the probes' sessions**, since `_latencies` runs inside the memory window but outside the power window. `partition_verified` and `fallback_reason` come from the batch result only, so a probe that partitioned differently is invisible.
- 6.3: peak resident memory is the maximum **current** working set during the cell, never the OS `PeakWorkingSetSize` - that is a high-water mark the OS never resets, so in one process measuring cells in sequence it pins every later cell to the largest earlier one. The cell-opening reading is published as `baseline_resident_memory` and **never subtracted**: the difference is not a model's memory cost, and reporting it as one would be requirement 6.8's forbidden estimate. A spike between two reads is under-measured rather than interpolated - the same stance `power.py` takes for a missed poll.
- 6.3: **ASSERT THE CLAIM, IN THE PLACE THE CLAIM LIVES, INCLUDING THE RELATION BETWEEN ITS PARTS.** This task found the membership-without-position defect at **four successive depths**, and the implementer, the reviewer and the controller each introduced one: (1) two independent memberships joined by `and`, where `"not"` was satisfied by a different sentence - so the central claim could be **inverted** with the suite green; (2) a fragment satisfied elsewhere in the same constant; (3) a regex matched against the whole clause rather than the term, so stripping one share still matched the other's percentage; (4) each figure bound to its own term but their **relation** unasserted, so the two shares can be transposed - routed to 6.7. Every one was found by planting the negation and none by reading. Prose is where this project puts its load-bearing caveats, because 6.8 forbids substituting estimates and stating the confound is the honest alternative - so **a loose assertion on documentation is worse than none: it looks like coverage while permitting the claim to vanish or reverse.**
- 6.3: pin methodology prose by **claim, not literal**. 7.2 re-measures on the NPU and will legitimately edit these strings, so an assertion on the number itself fires when someone does the right thing, which trains people to edit the test. Bind the figure to the term it quantifies (`session construction \(\d+(\.\d+)?%\)`) rather than asserting a literal or a bare pattern.
...
| 6.1 | — (none found; 32 mutants, zero survivors) | the one task whose fixture *was* the deliverable, and it held |
| 6.2 | `SERIES_INTERVAL = 0.5`, equal to the module default | the recorded interval hardcoded to `DEFAULT_SAMPLING_INTERVAL_SECONDS` survived all 52 unit tests |
| 6.2 | every sampler test used a 1 ms interval | a sampler ignoring its wait and polling in a tight loop survived all 52 — the perturbation the task exists to prevent |
| 6.2 | `assert reported_samples >= 1` on live hardware | not a fixture at all: an assertion that the *machine* cooperates, which fails ~3 runs in 4 under load, exactly when the code is right |
...
5. CI has no NPU, so a gap covered only by a live test is not covered at all. **This is now the dominant failure mode, not a footnote** — three of task 6.2's findings were live-only coverage or machine claims rather than wrong code.
6. **A live assertion that the hardware cooperates is not a test of your code** (6.2). Separate the two categories explicitly, as `test_power_live.py`'s docstring now does: properties of the code, which must also be pinned against captured fixture text, and claims about the machine, which must be named as such. When both outcomes of a machine's behaviour are correct, assert the code property under **both** rather than requiring the convenient one — that is strictly stronger, because it covers the branch nothing else reaches.
7. **Calibrate live tests on a busy machine, not an idle one** (6.2). An `xrt-smi` poll costs ~0.16 s idle and ~1.0 s under saturation; whole telemetry bursts come back empty 6 times in 8 under load versus 2 in 39 at idle. A benchmark measures a busy machine by definition, so idle-calibrated timing assumptions are calibrated on the wrong machine. Prefer waiting on the property (with a ceiling that fails loudly) over picking a constant, which is only ever a bet on machine speed.
8. **Run the gate's evidence fresh, and under load.** 6.2 was approved by an independent reviewer that ran the unit file six consecutive times and saw green; the flake surfaced only on the parent's own fresh full-suite run. Prior green runs are not evidence about the current tree.
9. **Assert the claim, in the place the claim lives, including the relation between its parts** (6.3). `x in s` reads as a stronger claim than it is: it cannot see position, cannot see negation, and can be satisfied by an unrelated sentence in the same blob. Task 6.3 found this at four successive depths and the implementer, the reviewer and the controller each introduced one — so treat it as a property of the construct, not of anyone's carelessness. Isolate the clause carrying the concept, assert the qualifier lives inside *it*, bind each figure to the term it quantifies, and where two parts must relate, assert the relation. Prove it by planting the negation: every instance was found by mutation and none by reading.
10. **Pin prose by claim, not by literal** (6.3). A methodology constant that a later task must legitimately re-measure should be guarded so a corrected figure passes while a deleted or inverted caveat fails. An assertion that fires when someone does the right thing trains people to edit the test.
...
3. **Export and NPU-compile it once** to confirm the path works end to end, as 3.2/3.3 did for MiniLM. ~~At ~600 MB it should compile faster than anything else in the set.~~ *(Measured 2026-09-07: **2953.5 s cold, 0.59 s warm.** The "compiles faster" expectation was wrong — see requirement 4.1's correction. Done; the path works end to end.)*
